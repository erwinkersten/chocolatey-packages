...
$releases = 'https://github.com/alexellis/k3sup/releases/latest'
...
function global:au_BeforeUpdate { Get-RemoteFiles -Purge -FileNameBase 'k3sup' -NoSuffix}
function global:au_GetLatest {
    $download_page = Invoke-WebRequest -Uri $releases -UseBasicParsing
    $artifact  = "k3sup.exe"
    $url = $download_page.links | Where-Object href -match $artifact | Select-Object -First 1 -expand href
    $version = ([regex]::Match($url,'/alexellis/k3sup/releases/download/(.+)/'+$artifact)).Captures.Groups[1].value
